...
<?php endif; ?>
  <div class="p-md-5 p-3 d-flex flex-column align-items-center">
    <h1 class="fw-bold"><?= !isset($menu) ? "Create" : "Edit" ?> Menu</h1>
    <form id="menu-item-form" action="<?= isset($menu) ? base_url("/business/menu/{$menu->menu_item_id}/edit") : base_url("/business/menu/create") ?>" method="post" enctype="multipart/form-data" class="w-100 d-flex flex-md-row flex-column justify-content-center gap-3">
      <input type="hidden" name="<?= csrf_token() ?>" value="<?= csrf_hash() ?>">
      <div class="w-mdc-25 w-md-50 w-100">
        <div class="card shadow">
          <img src='<?= isset($menu) && !is_null($menu->image_url) ? base_url("/business/menu/{$menu->menu_item_id}/image") : "https://theme-assets.getbento.com/sensei/7c1964e.sensei/assets/images/catering-item-placeholder-704x520.png" ?>' alt="" srcset="" id="menu-item-image-preview" class="rounded" style="min-height: 300px; max-height: 400px;">
        </div>
      ...
      <card class="card shadow w-mdc-50 w-100 p-3">
        <div class="mb-3">
          ...
          <input type="text" name="name" class="form-control bg-soft-gray" placeholder="Enter menu name here" value="<?= isset($menu) ? esc($menu->name) : "" ?>">
        </div>
        <label for="" class="form-label fw-bold">Price</label>
        <div class="input-group mb-3">
          <span class="input-group-text">AUD</span>
          <input type="number" step="any" name="price" class="form-control bg-soft-gray" placeholder="Enter price here" value="<?= isset($menu) ? number_format(esc($menu->price), 2, '.') : ""?>">
        </div>
        <div class="mb-3">
          <label for="" class="form-label fw-bold lh-sm">Category</label>
          <select name="category_id" class="form-select bg-soft-gray">
            <?php foreach ($categories as $category): ?>
              <option value="<?= esc($category->category_id) ?>" <?= isset($menu) ? ($menu->category_id === $category->category_id ? "selected" : "") : "" ?>><?= esc($category->name) ?></option>
            <?php endforeach; ?>
            <option value="others" <?= isset($menu) ? (is_null($menu->category_id) ? "selected" : "") : "" ?>>Others</option>
          </select>
        </div>
        <div class="mb-3">
          <label for="" class="form-label fw-bold">Description</label>
          <textarea class="form-control bg-soft-gray" id="description" name="description" rows="3" placeholder="Enter description here"><?= isset($menu) ? (is_null($menu->description) ? "" : esc($menu->description)) : "" ?></textarea>
        </div>
        <div class="form-check form-switch">
          <input type="checkbox" class="form-check-input" name="is_available" id="is_available" <?= isset($menu) ? ($menu->is_available ? "checked" : "") : "checked" ?>>
          <label for="" class="form-label">Product is available</label>
        </div>
        <div class="d-flex flex-row justify-content-end gap-1">
          <button type="submit" class="btn btn-<?= isset($menu) ? 'warning': 'success' ?>"><?= isset($menu) ? "Update " : "Create " ?>My Product</button>
          <?php if(isset($menu)): ?>
