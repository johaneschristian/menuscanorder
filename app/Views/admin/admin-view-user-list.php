  ...
  <div class="card shadow p-3 w-100 d-flex flex-md-row flex-column justify-content-between align-items-center">
    <form action="" method="get" class="w-mdc-50 w-75">
      <div class="input-group mb-md-0 mb-3 me-3 p-0">
        <input type="text" class="form-control bg-soft-gray" name="search" placeholder="Enter your search" value="<?= esc($search) ?>" />
        <button class="btn bg-brown text-white" type="submit" id="search-button">
          Search
        </button>
      </div>
    </form>
      ...
      <tbody>
        <?php foreach ($users as $key => $user) : ?>
          <tr>
            ...
            <td>
              <span id="user-user_id_1-status" class="badge rounded-pill bg-<?= $user->is_archived ? "warning text-dark" : "success" ?>"><?= $user->is_archived ? "archived" : "active" ?></span>
            </td>
            <td>
              <a href='<?= base_url("/admin/users/{$user->id}") ?>' class="btn btn-sm btn-primary me-2 mb-1">
                <i class="bi bi-eye-fill"></i>
              </a>
              <a href='<?= base_url("/admin/users/{$user->id}/edit") ?>' class="btn btn-sm btn-danger mb-1">
